...
DEFAULT_METADATA_TEMPLATE = {'version': 1, 'timestamp': 'YYYY-MM-DDTHH:MM:SS.SSSUTC', 'hash': '', 'system': {}}
...
class UDMIDiscovery():
    ...
    def __init__ (self, debug=DEFAULT_DEBUG_LEVEL, configuration_file=None, server=None, site=None, username=None, password=None, metatemplate=None):
        ...
        try:
            ...
            self.metadata_topic = '/devices/{GWID}/{DID}/meta'
        ...
        try:
            ...
            if self.configuration_file:
                ...
                try:
                    ...
                    self.use_tagging = True if self.config["use-tags"].lower() == 'true' else False
                    if "use-tags" in self.config:
                        self.use_tagging = True if self.config["use-tags"].lower() == 'true' else False
                        self.ontology = dbOntology()
                    else:
                        self.use_tagging = False

                    self.read_file_data = True if self.config["read-file-data"].lower() == 'true' else False

                    self.commissioned_flag = True if self.config["commissioned-flag"].lower() == 'true' else False
    ...
    def subscribe_gw_topics(self, client):
        ...
        client.publish(state_topic, "{'state': 'Connected'}",qos=0)
                ...
                for dev in devices:
                    ...
                    if str_dev_id.isnumeric():
                        ...
                        if not model_name["value"] in self.omit_devices:
                            ...
                            meta["localnet"]["subsystem"]["bacnet"]["local_id"] = str_dev_id
                            # Add BACnet Address and Timestamp to metadata
#                            meta["timestamp"] = datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ')
#                            meta["system"]["physical_tag"]["asset"]["name"] = str_dname
#                            meta["gateway"]["gateway_id"] = self.gateway_id
                                    ...
                                    if Commissioned:
                                        ...
                                        if (not self.use_tagging or (self.use_tagging and entity_found)) and not ex_found:
                                            ...
                                            if 'analog' in sp_obj[0].lower():
                                                # Call Get Request to retrieve units propertry for this object
                                            ...
                                            meta["pointset"]["points"][str_oname] = json.loads('{"ref" : "'+str_obj+sp_obj[1] + '.Present_Value","units": "' + str_units + '"}')
                                else:
                                    if dev == self.gateway_addr:
                                        if sp_obj[0] == 'file':
                                            # find 'Google Cloud IoT Gateway Public Key FIL object
                                            if objects[obj]["displayName"] == PUBLIC_KEY_CSV:
                                                # Write public key file in misc/{site}/devices/{gateway_id}
                                                print ("Public Key Object Found - {} ({})".format(objects[obj]["displayName"], sp_obj))
                                                if self.read_file_data:
                                                    #eWeb supports read data from FIL object
                                                    self.url = "{}/enteliweb/api/.bacnet/{}/{}/FIL,{}/.contents?alt=json".format(self.server, self.site, dev, sp_obj[1])
                                                    # Call Get Request to retrieve fil propertry for this object
                                                    res = self.get_from_api()
                                                    if res.status_code != 200:
                                                        self.printd("Failed to GET FIL .Contents property from eWeb API (Error = {})".format(res.status_code),level=0)
                                                        break
                                                    j_res = json.loads(res.text)
                                                    self.public_key = codecs.decode(j_res['value'][2:], 'hex').decode('utf-8')
                                                else:
                                                    #Public Key data should be in the Description property
                                                    self.url = "{}/enteliweb/api/.bacnet/{}/{}/FIL,{}/description?alt=json".format(self.server, self.site, dev, sp_obj[1])
                                                    # Call Get Request to retrieve fil propertry for this object
                                                    res = self.get_from_api()
                                                    if res.status_code != 200:
                                                        self.printd("Failed to GET Description property from eWeb API (Error = {})".format(res.status_code),level=0)
                                                        break
                                                    j_res = json.loads(res.text)
                                                    self.public_key = j_res["value"]
...
def parse_command_line_args():
    ...
    parser = argparse.ArgumentParser(description=(
            'Delta enteliWEB API based DAQ Discovery Tool for Google Cloud IoT Core.'))
    parser.add_argument("--debug", "-d", help="Debug output level 0,1,2,3", default=DEFAULT_DEBUG_LEVEL, const=0, nargs='?', type=int, choices=range(0, 4))
    parser.add_argument("--server", "-S", help="enteliWEB Server URL", default="http://localhost")
    parser.add_argument("--site", "-s", help="enteliWEB Site Name", default="Local")
    parser.add_argument("--username", "-n", help="enteliWEB user name", default='Partner')
    parser.add_argument("--password", "-p", help="enteliWEB  password", default='DemoomeD')
...
def main():
    ...
    try:
        ...
        eWeb = UDMIDiscovery(debug=args.debug, site=args.site,
                                    username=args.username, password=args.password,
                                    configuration_file=args.file, metatemplate=args.meta)
